...
public class Ble_grap_press extends BaseActivity {
	...
	protected void InitView() {
		setContentView(R.layout.ble_grap_press);
		
		mStart = (ToggleButton) findViewById(R.id.btn_press_start);
		sensor_sh_tv_show = (TextView) findViewById(R.id.sensor_tv_press_show);
	...
	protected void InitData() {
		...
		mCurrentSeries.add(x, PressData);
		++x;
		if (mChartView != null) {
	...
	protected void setChartSettings() {
		mRenderer.setChartTitle("压力传感器图表");
		mRenderer.setXTitle("时间(单位：s)");
		mRenderer.setYTitle("压力（单位：pa）");
	...
	private void initXYSeries() {
		...
		XYSeries series = new XYSeries("压力");// 定义XYSeries
	...
	public void onResume() {
		...
		if (mChartView == null) {
			LinearLayout layout = (LinearLayout) findViewById(R.id.grap_press);
	...
	protected BluetoothGattCharacteristic Event_ServicesFind() {
		return mBluetoothLeService.getWeNeed(
				UUID.fromString(GattAttributes.Press_Services),
				UUID.fromString(GattAttributes.Press_Data));
	}
	// 恢复状态
	@Override
	...
	protected void Event_HaveData(String data) {
		try {
			if (isStartOrNot) {
				PressData = Integer.valueOf(data, 16);
				mCurrentSeries.add(x, PressData);
				...
				sensor_sh_tv_show.setText("当前压力值：" + PressData + "Pa");
				++x;
			}
		} catch (Exception e) {
			e.printStackTrace();
